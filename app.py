...
if submit:
    # Normalizamos los valores según un rango máximo y mínimo de participación
    max_participacion = max(participaciones)
    min_participacion = min(participaciones)

    # Si la participación es 0, la normalizamos a un valor aceptable
    for i in range(5):
        if participaciones[i] == 0:
            participaciones[i] = max_participacion
    ...
    for i, jugador in enumerate(jugadores):
        jugador["Participación"] = participaciones[i]
